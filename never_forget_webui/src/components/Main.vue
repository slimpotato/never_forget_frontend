...
<template>
  <div>
        ...
        <div style="text-align: center; margin-top: 10px;">
          ...
          【语文】总数：{{total}} &nbsp; &nbsp;  
          两个字：{{w2_count}}/48  &nbsp; &nbsp; 三个字：{{w3_count}}/8 &nbsp; &nbsp;  四个字：{{w4_count}}/6
          ...
          <el-table 
            ...
            <el-table-column width="60" prop="section" sortable label="章节"></el-table-column>
...
export default {
  ...
  setup() {
    const mainData = ref([]);
    const total = ref(0);
    const w2_count = ref(0);
    const w3_count = ref(0);
    const w4_count = ref(0);
    ...
    const getWordCount = () => {
        ...
        //计算两个字，三个字，四个字的数量,读取mainData.value中object_content的长度
        let w2 = 0;
        let w3 = 0;
        let w4 = 0;
        for (let i = 0; i < mainData.value.length; i++) {
          if (mainData.value[i].object_content.length == 2) {
            w2++;
          } else if (mainData.value[i].object_content.length == 3) {
            w3++;
          } else if (mainData.value[i].object_content.length == 4) {
            w4++;
          }
        }
        w2_count.value = w2;
